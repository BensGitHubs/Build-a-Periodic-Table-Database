...
if [[ ! $ELEMENT_ARG ]];  then
    ...
    # Look up the argument in the Database to find a matching element
    # Need to join tables and search in multiple columns for match - atomic weight, name or symbol

    PATTERN='^[1-9][0-9]*$'
    if [[ "$ELEMENT_ARG" =~ $PATTERN   ]]
    then
    # Argument is a number so could be an atomic number of an element in database
